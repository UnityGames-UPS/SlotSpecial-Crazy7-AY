...
public class UIManager : MonoBehaviour
{
    ...
    {
        slotManager.coinAnimator.StopCoinAnimation();
        ImageAnimation ig = Win_Image.GetComponent<ImageAnimation>();
        ig.textureArray.Clear();
        ig.textureArray.TrimExcess();
        switch (value)
        {
            case 1:
                for (int i = 0; i < BigWin_Sprite.Length; i++)
                {
                    ig.textureArray.Add(BigWin_Sprite[i]);
                }
                ig.AnimationSpeed = 30f;
                break;
            case 2:
                for (int i = 0; i < HugeWin_Sprite.Length; i++)
                {
                    ig.textureArray.Add(HugeWin_Sprite[i]);
                }
                ig.AnimationSpeed = 30f;
                break;
            case 3:
                for (int i = 0; i < MegaWin_Sprite.Length; i++)
                {
                    ig.textureArray.Add(MegaWin_Sprite[i]);
                }
                ig.AnimationSpeed = 30f;
                break;
            case 4:
                ...
                {
                    ig.textureArray.Add(Jackpot_Sprite[i]);
                }
                ig.AnimationSpeed = 30f;
                break;
        }
    ...
    public void OnClickPayTableButtons(int id)
    {
        ...
        foreach (var item in PaytablePages)
        {
            item.SetActive(false);
        }

        PaytablePages[id].SetActive(true);
        PaytableButtonsImages[id].SetActive(true);
    }
    private void PopulateSymbolsPayout(Paylines paylines)
    {
        ...
        for (int i = 0; i < SymbolsText.Length; i++)
        {
            ...
            {
                if (SymbolsText[i]) SymbolsText[i].text = paylines.symbols[i].description.ToString();
            }
        }
